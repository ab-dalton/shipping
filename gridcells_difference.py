# -*- coding: utf-8 -*-
"""
Created on Tue Apr 25 09:20:23 2023
...
# ----------------------------------------------------------------------------
# Calculate percent difference between early and late
# ----------------------------------------------------------------------------
...
for vessel_type in vessel_types:
    ...
    # Calculate the counts for early and late seasons
    early_count = merged_mmsi_early[vessel_type]
    late_count = merged_mmsi_late[vessel_type]
    
    # Calculate the percent difference for each grid cell
    pct_diff = ((late_count - early_count) / early_count) * 100
    
    # Add the percent difference to the dataframe
    percent_diff[vessel_type] = pct_diff
    
# Add a column for the total percent difference across all vessel types
percent_diff['ALL_TYPES'] = percent_diff.sum(axis=1)

percent_diff = percent_diff.loc[(percent_diff['ALL_TYPES'] != 0)]


merged_percent_diff = pd.merge(grid, percent_diff, left_index=True, right_index=True, how="outer")

# ----------------------------------------------------------------------------
# Calculate absolute difference between early and late
# ----------------------------------------------------------------------------
...
for vessel_type in vessel_types:
    ...
    # Calculate the percent difference for each grid cell
    abs_diff = (late_count - early_count)
    
    # Add the percent difference to the dataframe
    absolute_diff[vessel_type] = abs_diff
    
# Add a column for the total percent difference across all vessel types
absolute_diff['ALL_TYPES'] = absolute_diff.sum(axis=1)

absolute_diff = absolute_diff.loc[(absolute_diff['ALL_TYPES'] != 0)]

merged_absolute_diff = pd.merge(grid, absolute_diff, left_index=True, right_index=True, how="outer")
...
norm_percent = mpl.colors.Normalize(vmin=-100, vmax=2000) #50
norm_absolute = mpl.colors.Normalize(vmin=-50, vmax=100) #50
...
cb_absolute = fig.colorbar(cm.ScalarMappable(norm=norm_absolute, cmap=cmap),
                  ax=axs[0],
                  orientation='horizontal') 
cb_absolute.ax.tick_params(labelsize=12)
cb_absolute.set_label('Absolute Difference', fontsize=14)

cb_percent = fig.colorbar(cm.ScalarMappable(norm=norm_percent, cmap=cmap),
                  ax=axs[1],
                  orientation='horizontal') 
cb_percent.ax.tick_params(labelsize=12)
cb_percent.set_label('Percent Difference', fontsize=14)
