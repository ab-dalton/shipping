# -*- coding: utf-8 -*-
"""
Created on Wed May  3 09:01:41 2023
...
stats_long_res_time = joined_early_beaconid.groupby(["index_right", "beacon_id"])["datetime_data"].agg(
    ["min", "max"]
)
...
# Merge dataframes to add statistics to the polygon layer
merged_early_duration = pd.merge(grid, stats_res_time, left_index=True, right_index=True, how="outer")

# Summarize the stats for each attribute in the point layer - residence time LATE
stats_res_time = joined_late_beaconid.groupby(["index_right"])["speed_ms"].agg(
    ["min", "max"]
)

stats_long_res_time = joined_late_beaconid.groupby(["index_right", "beacon_id"])["datetime_data"].agg(
    ["min", "max"]
)
...
# Merge dataframes to add statistics to the polygon layer
merged_late_duration = pd.merge(grid, stats_res_time, left_index=True, right_index=True, how="outer")
...
norm = mpl.colors.Normalize(vmin=1, vmax=30) #50
